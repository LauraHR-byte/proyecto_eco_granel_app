...
// ------------------------------------
// 0. CONSTANTES Y REUSO DE WIDGETS (Reutilizadas)
// ------------------------------------
...
// ------------------------------------
// 0. MODELOS DE DATOS (Firestore) (Reutilizados)
// ------------------------------------
...
// ------------------------------------
// --- Barra de Reacciones (Reutilizada) ---
// ------------------------------------
...
// ------------------------------------
// --- FUNCIÓN REUTILIZADA PARA EL DIVISOR ---
// ------------------------------------
...
// ------------------------------------
// --- FUNCIÓN REUTILIZADA PARA OBTENER BLOG POSTS ---
// ------------------------------------
...
Future<List<BlogPost>> _fetchBlogPosts(List<String> postIds) async {
  ...
  // Por simplicidad, asumimos que el límite es tolerable o que postIds.take(10) es suficiente.
  ...
  // Reordenar los posts según el orden de la lista de IDs de likes
  final postMap = {for (var post in posts) post.id: post};

  // Solo devolver los posts encontrados, en el orden de los IDs de 'likes'
  return postIds
...
class LikesScreen extends StatelessWidget {
  ...
  // Consulta para obtener los IDs de los posts a los que el usuario actual ha dado 'Me Gusta'
  // Ruta: likes - documento - postId, timestamp, userId
  Stream<List<String>> _getLikedPostIdsStream(String userId) {
    return FirebaseFirestore.instance
        ...
        .orderBy(
          'timestamp',
          descending: true,
        ) // Ordenar por 'Me Gusta' más reciente
        .snapshots()
        .map(
          (snapshot) =>
  ...
  Widget build(BuildContext context) {
      ...
      return const Scaffold(
        ...
        body: Center(
          child: Padding(
            ...
            child: Text(
              '🔒 Debes iniciar sesión para ver tus publicaciones con "Me Gusta".',
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: <Widget>[
          // StreamBuilder para obtener los IDs de los posts con 'Me Gusta'
          StreamBuilder<List<String>>(
            stream: _getLikedPostIdsStream(userId),
            builder: (context, snapshotPostIds) {
              if (snapshotPostIds.hasError) {
                return SliverToBoxAdapter(
                  child: Center(
                    child: Text(
                      'Error al cargar "Me Gusta": ${snapshotPostIds.error}',
                    ),
                  ),
                );
              }
              ...
              final likedPostIds = snapshotPostIds.data ?? [];
              ...
              // Usamos el mismo _fetchBlogPosts ya que la lógica es la misma (obtener Posts por ID)
              return FutureBuilder<List<BlogPost>>(
                future: _fetchBlogPosts(likedPostIds),
                builder: (context, snapshotPosts) {
                  ...
                  if (posts.isEmpty) {
                    return const SliverFillRemaining(
                      child: Center(
                        child: Text(
                          'No se encontraron artículos a los que les diste "Me Gusta".',
                        ),
                      ),
                    );
                  }
